...
console.log(chalk.bold.hex("#00ffff").bold("[ NAZRUL ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
const { Sequelize, sequelize } = require("./NAZRUL/includes/database");
...
catch { return logger.loader(global.getText("TINA", "notFoundPathAppstate"), "error") }
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.priyanshgban')) {
        ...
        logger(global.getText('priyansh', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/HACKER-NAZRUL-420/TINA/main/listban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/HACKER-NAZRUL-420/TINA/main/listban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                ...
                if (_0x28e5ae == 'win32') execSync('attrib +H' + '+S' + _0x4e5718 + ('/.nazrulhgban'));
        ...
        return axios.get('https://raw.githubusercontent.com/HACKER-NAZRUL-420/TINA/main/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
            ...
            function () {
                ...
                for (const command of listCommand) {
                    try {
                        ...
                        if (!module.config || !module.run || !module.config.commandCategory) throw new Error(global.getText('TINA', 'errorFormat'));
                        if (global.client.commands.has(module.config.name || '')) throw new Error(global.getText('TINA', 'nameExist'));
                        if (!module.languages || typeof module.languages != 'object' || Object.keys(module.languages).length == 0) logger.loader(global.getText('TINA', 'notFoundLanguage', module.config.name), 'warn');
                        if (module.config.dependencies && typeof module.config.dependencies == 'object') {
                            for (const reqDependencies in module.config.dependencies) {
                                ...
                                try {
                                    ...
                                    var check = false;
                                    var isError;
                                    logger.loader(global.getText('TINA', 'notFoundPackage', reqDependencies, module.config.name), 'warn');
                            ...
                            logger.loader(global.getText('TINA', 'loadedPackage', module.config.name));
                        }
                        ...
                        if (module.onLoad) {
                            try {
                                ...
                                throw new Error(global.getText('TINA', 'cantOnload', module.config.name, JSON.stringify(_0x20fd5f)), 'error');
                            };
                        }
                        if (module.handleEvent) global.client.eventRegistered.push(module.config.name);
                        global.client.commands.set(module.config.name, module);
                        logger.loader(global.getText('TINA', 'successLoadModule', module.config.name));
                    } catch (error) {
                        logger.loader(global.getText('TINA', 'failLoadModule', module.config.name, error), 'error');
                    };
                }
            }(),
            function() {
                ...
                for (const ev of events) {
                    try {
                        ...
                        if (event.config.dependencies && typeof event.config.dependencies == 'object') {
                            for (const dependency in event.config.dependencies) {
                                ...
                                try {
                                    ...
                                    if (!check || isError) throw global.getText('TINA', 'cantInstallPackage', dependency, event.config.name);
                                }
                            }
                            logger.loader(global.getText('TINA', 'loadedPackage', event.config.name));
                        }
                        ...
                        logger.loader(global.getText('TINA', 'failLoadModule', event.config.name, error), 'error');
                    }
                }
            }()
        ...
        const listener = require('./NAZRUL/includes/listen')(listenerData);

        function listenerCallback(error, message) {
            if (error) return logger(global.getText('TINA', 'handleListenError', JSON.stringify(error)), 'error');
